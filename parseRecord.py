...
import sys, os
import argparse
from stats import PA, Batter, Pitcher
...
def ParseGameData(record):
    ...
    for i in range(nPlayer):
        player.append( Batter(record[i][0].upper(), record[i][1].upper()) ) # order, number
        n = len(record[i])
    ...
    while(True):
        pa = ParsePAstring(record[order][turn+2].upper())
